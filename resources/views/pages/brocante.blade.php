  ...
  <div class="md:col-span-full lg:col-span-6 lg:col-start-4 md:pb-64">
    <div class="md:grid md:grid-cols-12 md:gap-x-16 md:gap-y-64 space-y-64 md:space-y-0">
      @foreach($products->sortBy('sort') as $product)
        <x-product.cards.product :product="$product" :category="$product->product_category_id" :lightbox="true" />
        @if ($product->variations->count() > 0)
          @foreach($product->variations as $variation)
            <x-product.cards.product :product="$variation" :category="$product->product_category_id" :lightbox="true" />
          @endforeach
        @endif
      @endforeach
    </div>
